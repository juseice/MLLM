...
def main():
    ...
    target_entry = None
    for entry in data:
        if entry.get('gamma') == gamma and entry.get('delta') == delta:
            target_entry = entry
            break
    ...
    for sample in tqdm(samples, desc="Detecting"):
        ...
        # --- LOAD DETECTOR ---
        # We re-init AutoWatermark for each sample because the hash_key changes
    ...
    print("\n" + "="*40)
    print(f"Attack Analysis Results (gamma={gamma}, delta={delta})")
    print("="*40)
    ...
    output_path = os.path.join(args.output_dir, f"analysis_{gamma}_{delta}.json")
