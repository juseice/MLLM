...
with torch.no_grad():
    for i, line in enumerate(tqdm(lines, desc=f"δ={delta}", unit="line")):
        ...
        BLEU_watermark.append(bleu_calculator.analyze(wm_text,prompt))
        BLEU_nowatermark.append(bleu_calculator.analyze(nwm_text,prompt))
...
results_name = f"T5_C4_5k_SIR_{delta}_20260204"
